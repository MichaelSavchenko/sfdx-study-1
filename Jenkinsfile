...
node {
    ...
    withCredentials([file(credentialsId: 'SERVER_KEY', variable: 'jwt_key_file')]) {
        ...
        stage('Create Test Scratch Org') {
                rc = sh returnStatus: true, script: "${toolbelt}/sfdx force:org:create --setdefaultusername --definitionfile config/project-scratch-def.json --setalias ciorg --wait 10 --durationdays 1"
                if (rc != 0) {
                    println rc
